...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
	...
	Pde *pgdir;
	ret = 0;
	if ( ((perm & PTE_COW) != 0) || ((perm & PTE_V) == 0) || va >= UTOP) return -E_INVAL; 
	if ((ret = envid2env(envid, &env, 1)) != 0) return ret;
	pgdir = env->env_pgdir;
	if ((ret = page_alloc(&ppage)) != 0) return ret;
	if ((ret = page_insert(pgdir, ppage, va, perm)) != 0) return ret;
	return ret;
}
				...
				u_int perm)
{
	...
	if ((ret = envid2env(srcid, &srcenv, 0)) != 0) return ret; //??? last parameter equals 1 or 0
	if ((ret = envid2env(dstid, &dstenv, 0)) != 0) return ret; //??? last parameter equals 1 or 0
	ppage = page_lookup(srcenv->env_pgdir, round_srcva, &ppte);
	if (ppage == 0) return -1;
	if ((perm & PTE_R) != 0 && (*ppte & PTE_R) == 0) return -E_INVAL;
...
int sys_set_env_status(int sysno, u_int envid, u_int status)
{
	...
	if ((ret = envid2env(envid, &env, 1)) != 0) return ret; //??? 1 or 0
					 ...
					 u_int perm)
{
	...
	if (srcva) {
		...
		if (p == 0) return -E_IPC_NOT_RECV;
		if ((r = page_insert(e->env_pgdir, p, e->env_ipc_dstva, perm)) != 0) return -E_IPC_NOT_RECV;
	} 
	return 0;
}
