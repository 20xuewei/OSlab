...
diskaddr(u_int blockno)
{
	if (super && blockno >= super->s_nblocks) {
		user_panic("Error in fs.c/diskaddr,reading non-existent block %08x\n", blockno);
	}
...
unmap_block(u_int blockno)
{
	...
	if(syscall_mem_unmap(0, diskaddr(blockno)) != 0) user_panic("fs.c: unmap_block syscall_mem_unmap\n");
...
free_block(u_int blockno)
{
	...
	if (blockno == 0 || super == 0 || blockno >= super->s_nblocks) {
		//user_panic("fs.c: free_block -E_INVAL\n");
		return;
	}
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	for (i = 0; i < nblock; i++) {
		...
		if ((r = file_get_block(dir, i, &blk)) < 0) {
			writef("fs.c: dir_lookup file_get_block\n");
			return r;
		}
