...
pgfault(u_int va)
{
	...
	if (!(perm & PTE_COW)) {
		user_panic("pgfault: do not have COW\n");
		return;
	}
    //map the new page at a temporary place
	if (syscall_mem_alloc(0, tmp, PTE_R | PTE_V) != 0) {
		user_panic("pgfault: mem alloc fault\n");
		return;
	}
	//copy the content
	...
	if (syscall_mem_map(0, tmp, 0, va, PTE_V | PTE_R) != 0) {
		user_panic("pgfault: mem map fault\n");
		return;
	}
    //unmap the temporary place
	if (syscall_mem_unmap(0, tmp) != 0) {
		user_panic("pgfault: mem unmap fault\n");
		return;
	}
...
duppage(u_int envid, u_int pn)
{
	...
	addr = pn * BY2PG;
	perm = (*vpt)[pn] & 0xfff;
	if (!(perm & PTE_R) || !(perm & PTE_V) || (perm & PTE_LIBRARY) || (perm & PTE_COW)) {
		if ((ret = syscall_mem_map(0, addr, envid, addr, perm)) != 0) {
			user_panic("duppage: mem map fault\n");
			return;
		}
	}
	else {
		...
		if ((ret = syscall_mem_map(0, addr, 0, addr, perm)) != 0) {
			user_panic("duppage: mem map fault\n");
			return;
		}
	}
