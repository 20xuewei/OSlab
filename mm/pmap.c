...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
	...
	if (create) {
		pgtable = (Pde *)alloc(BY2PG, BY2PG, 1);
...
page_init(void)
{
    ...
    int i;
    for (i=0; i<num; i++) pages[i].pp_ref = 1;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if (create) {
		if (page_alloc(&ppage) == 0) {
			ppage->pp_ref++;
			(*pgdir_entryp) = (u_long)(page2pa(ppage));
			(*pgdir_entryp) |= PTE_V|PTE_R;
			pgtable = KADDR((u_long)(page2pa(ppage)));
		}
		else {
			*ppte = NULL;
			return -E_NO_MEM;
